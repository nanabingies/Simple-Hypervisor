...
namespace vmexit {
	auto vmexit_handler(void* args) -> void {
		auto guest_regs = reinterpret_cast<__guest_registers*>(args);
	...
	auto handle_ept_violation(void* args) -> void {
		...
		size_t guest_phys_addr, guest_linear_addr;
		__vmx_vmread(VMCS_GUEST_PHYSICAL_ADDRESS, &guest_phys_addr);
		//phys_addr = PAGE_ALIGN((PVOID)phys_addr);
		__vmx_vmread(VMCS_EXIT_GUEST_LINEAR_ADDRESS, &guest_linear_addr);

		LOG("[*] EPT violation at address %llx [phys : %llx]\n", guest_linear_addr, guest_phys_addr);
		LOG("[*] read : %llx | write : %llx | execute : %llx\n", exitQualification.ept_readable, exitQualification.ept_writeable, exitQualification.execute_access);
	...
	auto handle_ept_misconfiguration(void* args) -> void {
		...
		__vmx_vmread(VMCS_GUEST_PHYSICAL_ADDRESS, &guest_phys_addr);
		__vmx_vmread(VMCS_EXIT_GUEST_LINEAR_ADDRESS, &guest_linear_addr);
		LOG("[*] EPT Misconfiguration at address %llx [phys : %llx]\n", guest_linear_addr, guest_phys_addr);
